    ...
    stages {
        ...
        stage('Deploy the Application to Kubernetes') {
            steps {
                ...
                sh 'scp -o StrictHostKeyChecking=no -i ./terraform-files/virginia.pem ./terraform-files/kube.yml ubuntu@54.88.197.190:/home/ubuntu/'
                sh 'scp -o StrictHostKeyChecking=no -i ./terraform-files/virginia.pem ./terraform-files/service.yml ubuntu@54.88.197.190:/home/ubuntu/'

                script {
                    try {
                        // Apply the Kubernetes configuration
                        sh 'ssh -o StrictHostKeyChecking=no -i ./terraform-files/virginia.pem ubuntu@54.88.197.190 "kubectl apply -f /home/ubuntu/kube.yml"'
                        sh 'ssh -o StrictHostKeyChecking=no -i ./terraform-files/virginia.pem ubuntu@54.88.197.190 "kubectl apply -f /home/ubuntu/service.yml"'
                    } catch (error) {
